...
public class TodayFragment extends Fragment implements ScheduleAdapter.OnScheduleClickListener, SearchableFragment {
    ...
    private void loadSchedules() {
        if (!isAdded()) return;
        String today = DateTimeUtils.getCurrentDate(requireContext());
        List<Schedule> todaySchedules = dbHelper.searchSchedules(currentQuery, currentCategory, currentStatus, currentPriority, currentSortBy, today);
        
        adapter.submitList(todaySchedules);
        tvEmpty.setVisibility(todaySchedules.isEmpty() ? View.VISIBLE : View.GONE);
        if (todaySchedules.isEmpty() && !currentQuery.isEmpty()) {
